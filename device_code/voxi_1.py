    ...
    for name in targets:
        for device_name, paths in devices.items():
            if device_name.startswith(name):
                ...
                if first_video:
                    # first_video is like /dev/video0; keep it as an int device id.
                    try:
            ...
            try:
                ...
                if k & 0xFF == ord("v"):
                    ...
                    if doRecordVideo:       
                        ...
                        video_session_dir = os.path.join(VideoSaveDir, f"voxi1_session_{current_time}")
                        os.makedirs(video_session_dir, exist_ok=True)
                        print(f'VOXI 1 is now recording')

                    else:
                        print(f'VOXI 1 STOPPED RECORDING')
